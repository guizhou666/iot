<template>
  <div class="access-wrapper">
    <AppMainHeader title="资源配置" hasBackBtn></AppMainHeader>
...
export default {
  ...
  methods: {
    /**
     * @description:
     * 查询全部资源，根据search判断是不是第一次查询，
     * search为空代表第一次查询需要先查当前产品的已关联资源
     * 否则 是通过点击查询按钮查询，
     * @param {*} search 用来判断是否通过点击查询按钮触发
     * @return {*}
     */
    ...
    async confirm() {
      ...
      if (code === '200') {
        this.$message({
          type: 'success',
          message: this.tab == '1' ? '关联资源成功' : '取消关联成功'
        })
